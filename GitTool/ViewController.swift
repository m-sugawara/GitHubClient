...
class ViewController: OSViewController {
    #if TRACE_RESOURCES
    #if !RX_NO_MODULE
    private let startResourceCount = RxSwift.Resouces.total
    #else
    ...
    deinit {
             ...
             * navigate back
             * observe intial resource count
             * navigate second time to your screen and use it
             * navigate back
             * observe final resource count
             
             in case there is a difference in resource count between initial and final resource counts, there might be memory leak somewhere.
             
             The reason why 2 navigations are suggested is because first navigation forces loading of lazy resources
             
             */
            
            let numberOfResourcesThatSouldRemain = startResourceCount
            let mainQueue = DispatchQueue.main
            /*
             This first `dispatch_async` is here to compensate for CoreAnimation delay after changing view controller hierarchy. This time is usually ~100ms on simulator and less on device.
            ...
            mainQueue.asyncAfter(deadline: when) {
                ...
                /*
                 Some small additional period to clean things up. In case there were async operations fired, they can't be cleaned up momentarily.
                 */
                // If this falls for you while testing, and you've ben clicking fast, it's ok, just click slower,
                ...
                assert(Resources.total <= numberOfResourcesThatSouldRemain, "Resources weren't cleaned properly, \(Resource.total) remaned, \'(numberOfResourcesThatShouldRemaain) expected")
            }
